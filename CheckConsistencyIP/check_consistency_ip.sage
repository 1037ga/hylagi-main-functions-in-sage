...
def check_consistency_Ip_qe(constraint_store, symbolic_parameters, prevs, vars):
    qf = qepcad_formula
    symbolic_parameters_conj = __convert_to_conjunctive(symbolic_parameters[0])
    # st = desolve_system(constraint_store[0],vars, ics=[0]+prevs)
    # st = list(map(lambda e,f: f == (e.rhs()).subs(t == 0), st, vars))
...
def main():
    ...
    (y, dy, ddy, p_y, t) = var('y dy ddy p_y t')
    ...
    timer = cputime()
    (consistency, updated_parameters) = check_consistency_Ip_qe(constraint_store, symbolic_parameters, prevs, vars)
    print("==== output =====")
